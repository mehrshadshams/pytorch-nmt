...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in range(self.num_epoch):
            ...
            for i, batch in enumerate(tqdm(self.train_loader)):
                ...
                loss = criterion(decoder_logit, trg_output.contiguous().view(-1))
                loss.backward()
    ...
    def print_sample(self, batch_size, epoch, train_iter, source, target, pred):
        ...
        random_idx = _randomChoice(batch_size)
    ...
    def get_sentence(self, sentence, side):
        def _eos_parsing(sentence):
            if EOS_WORD is sentence:
                return sentence[:sentence.index(EOS_WORD) + 1]
            else:
                return sentence

        inv_vocab = self.trg_inv_vocab if side == 'trg' else self.src_inv_vocab
        sentence = [inv_vocab[x] for x in sentence]
    ...
    def build_model(self, vocabs):
        ...
        return Seq2Seq(self.src_nword, self.trg_nword, self.num_layer, self.embed_dim, self.hidden_dim,
